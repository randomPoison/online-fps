...
fn main() -> amethyst::Result<()> {
    static SHUTDOWN_IO_THREAD: ::std::sync::atomic::AtomicBool = ::std::sync::atomic::AtomicBool::new(false);
    ...
    let display_config = DisplayConfig::load(
        concat!(env!("CARGO_MANIFEST_DIR"), "/resources/display.ron"),
    );
    ...
    let sync_editor = SyncEditorBundle::new()
        ...
        //
        // TODO: Build registration for engine types into amethyst-editor-sync.
        ...
        // .sync_component::<MeshHandle>("Mesh Handle")

        // Register types for the game.
        .sync_component::<Player>("Player")
        .sync_component::<InputFrame>("Input Frame")
        .sync_component::<RevolverEntities>("Revolver Entities")
        .sync_component::<BipedEntities>("Biped Entities")
        .sync_component::<LocalPlayer>("Local Player")
        .sync_component::<PlayerEntities>("Player Entities")
        .sync_component::<PlayerPitch>("Player Pitch")
        .sync_resource::<FrameId>("Frame ID");
        // .sync_resource::<GltfCache>("glTF Cache")
    ...
    let game_data = GameDataBuilder::default()
        ...
        .with(LateInitSystem::<BipedEntities>::default(), "biped_entities_init", &["gltf_loader"])
        .with(LateInitSystem::<RevolverEntities>::default(), "revolver_entities_init", &["gltf_loader"])
        .with(HideBodySystem::default(), "hide_body", &["biped_entities_init"])
        .with_barrier()
    ...
    let io_thread = thread::spawn(move || {
        ...
        let wait_for_connection = ::sumi::Connection::connect(address, &core.handle())
            ...
            .map(move |connection| ::core::Connection::<ClientMessage, ServerMessage>::new(connection, &handle))
...
fn build_player(
    ...
    updater.insert(root, PlayerEntities { head: head.into(), gun: gun.into() });
    ...
    assert_eq!(old, None, "There was already an entity for player ID {:#x}", id);
}
